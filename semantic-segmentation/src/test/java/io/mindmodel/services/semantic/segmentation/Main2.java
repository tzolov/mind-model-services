...
public class Main2 {
	...
	public static Tensor<?> boza() throws IOException {
		SegmentationInputConverter inputConverter = new SegmentationInputConverter();
		Map<String, Tensor<?>> inTensors = inputConverter.apply(GraphicsUtils.loadAsByteArray("file:/Users/ctzolov/Dev/projects/mindmodel/mind-model-services/semantic-segmentation/src/test/resources/images/VikiMaxiAdi.jpg"));
		TensorFlowService service = new TensorFlowService(new DefaultResourceLoader().getResource("http://download.tensorflow.org/models/deeplabv3_mnv2_pascal_trainval_2018_01_29.tar.gz#frozen_inference_graph.pb")
...
//		byte[] inputImage = GraphicsUtils.loadAsByteArray("classpath:/images/giant_panda_in_beijing_zoo_1.jpg");
	...
	public static List<? extends Tensor<?>> compute3() throws IOException {
		SegmentationInputConverter inputConverter = new SegmentationInputConverter();
		Map<String, Tensor<?>> inTensors = inputConverter.apply(GraphicsUtils.loadAsByteArray("file:/Users/ctzolov/Dev/projects/mindmodel/mind-model-services/semantic-segmentation/src/test/resources/images/VikiMaxiAdi.jpg"));
		Tensor<UInt8> normalizedImage = inTensors.get(SemanticSegmentationUtils.INPUT_TENSOR_NAME).expect(UInt8.class);
		TensorFlowService service = new TensorFlowService(new DefaultResourceLoader().getResource("http://download.tensorflow.org/models/deeplabv3_mnv2_pascal_trainval_2018_01_29.tar.gz#frozen_inference_graph.pb")
				, Arrays.asList(SemanticSegmentationUtils.OUTPUT_TENSOR_NAME), true);
		Map<String, Tensor<?>> outTensors = service.apply(inTensors);


		Tensor<Long> maskTensor = outTensors.get(SemanticSegmentationUtils.OUTPUT_TENSOR_NAME).expect(Long.class);
		...
		ExpandDims<Long> expanded = tf.withName("expanded").expandDims(input, tf.rank(input));

		Cast<Float> b = tf.dtypes.cast(expanded, Float.class);
		Operand<Float> nm = normalizeMask(tf, b, 255f);
		Operand<Float> irgb = grayscaleToRgb(tf, nm);
		Squeeze<Float> sq = tf.squeeze(irgb, Squeeze.axis(Arrays.asList(0L)));

		// Remove the batch dimension. e.g. From [1,513,401,3] to [513,401,3]
		Operand<String> png = tf.withName("png").image.encodePng(tf.dtypes.cast(sq, UInt8.class));

		Session s = new Session(g);

		List<Tensor<?>> result = s.runner()
				.feed("input_tensor", maskTensor)
				.fetch("png")
				.run();

		System.out.println(result);

		Tensor<?> maskAsRgbTensor = result.get(0);

		byte[] mask = maskAsRgbTensor.bytesValue();

		ByteArrayInputStream bis = new ByteArrayInputStream(mask);
		BufferedImage bImage2 = ImageIO.read(bis);
		ImageIO.write(bImage2, "png", new File("./semantic-segmentation/target/outputMask2.png"));

		s.close();
		return Arrays.asList(normalizedImage, result.get(1));
	}
